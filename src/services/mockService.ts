...
export const MockService = {
  ...
  getNeighborhoods: async (forceRefresh = false): Promise<Neighborhood[]> => {
    const demoHoods = [
      ...
      },
      {
        ...
        description: 'Bairro monitorado com câmeras de alta definição e ronda ativa.',
        iframeUrl: 'about:blank',
        lat: -27.6012,
        lng: -48.5410
      },
      {
        id: 'hood-demo-3',
        name: 'Porto Seguro',
        description: 'Bairro litorâneo com monitoramento patrimonial ativo.',
        iframeUrl: 'about:blank',
        lat: -27.5850,
        lng: -48.5600
      }
  ...
  createNeighborhood: async (name: string, description: string, iframeUrl: string): Promise<void> => {
    const newId = `hood-${Date.now()}`;
    const newHood: Neighborhood = {
        id: newId,
        name,
        description,
        iframeUrl,
    ...
    try {
        const list = await MockService.getNeighborhoods();
        const updated = [...list.filter(h => h.id !== newId), newHood];
  ...
  updateNeighborhood: async (id: string, name: string, description: string, iframeUrl: string): Promise<void> => {
    try {
        const list = await MockService.getNeighborhoods();
        const updated = list.map(h => h.id === id ? { ...h, name, description, iframeUrl } : h);
        localStorage.setItem('atalaia_local_neighborhoods', JSON.stringify(updated));
    } catch (err) {}
    ...
    try {
        ...
        if (typeof window !== 'undefined') {
            ...
            try {
                ...
                console.warn("[getPayments Fallback] Could not enrich with mock profiles:", userErr);
            }
        }
        return localPayments;
    }
